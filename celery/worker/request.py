...
class Request:
        ...
        **opts,
        ...
        if eta is not None:
            try:
                ...
                raise InvalidTaskError(f"invalid ETA value {eta!r}: {exc}") from exc
        ...
        if expires is not None:
            try:
                ...
                raise InvalidTaskError(f"invalid expires value {expires!r}: {exc}") from exc
    ...
    def __repr__(self):
        """``repr(self)``."""
        return f"<{type(self).__name__}: {self.humaninfo()} {self._argsrepr} {self._kwargsrepr}>"
