...
class ColorFormatter(logging.Formatter):
    ...
    def __init__(self, fmt=None, use_color=True, datefmt=None):
        # `datefmt` goes last rather than in logging.Formatter's position, so
        # the existing positional call `ColorFormatter(fmt, False)` still means
        # "no colour" and not "no date format" (upstream e7c4454f8).
        ...
        if color and self.use_color:
            try:
                ...
                prev_msg, record.exc_info, record.msg = (
                    record.msg,
                    1,
                    f"<Unrepresentable {type(msg)!r}: {exc!r}>",
                )
                try:
