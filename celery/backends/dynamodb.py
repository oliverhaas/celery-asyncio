...
class DynamoDBBackend(KeyValueStoreBackend):
    ...
    def __init__(self, url=None, table_name=None, *args, **kwargs):
        ...
        if url is not None:
            ...
            if region == "localhost" or DynamoDBBackend._is_valid_ip(region):
                ...
                logger.warning(f"Using local-only DynamoDB endpoint URL: {self.endpoint_url}")
            else:
    ...
    def _get_or_create_table(self):
        ...
        try:
            ...
            if error_code == "ResourceNotFoundException":
                ...
                logger.info(f"DynamoDB Table {self.table_name} did not exist, creating.")
                # In case we created the table, wait until it becomes available.
                self._wait_for_table_status("ACTIVE")
                logger.info(f"DynamoDB Table {self.table_name} is now available.")
    ...
    def _get_table_ttl_description(self):
        ...
        try:
            ...
            logger.error(
                f"Error describing Time to Live on DynamoDB table {self.table_name}: {error_code}: {error_message}"
            )
            raise e
    ...
    def _set_table_ttl(self):
        ...
        # a log showing the unknown state.
            ...
            logger.warning(
                f"Unknown DynamoDB Time to Live status {status} on table {self.table_name}. Attempting to continue."
            )
        ...
        try:
            ...
            logger.info(
                ...
                f"table={self.table_name} enabled={self._has_ttl()} attribute={self._ttl_field.name}"
            )
    ...
    def _wait_for_table_status(self, expected="ACTIVE"):
        ...
        while not achieved_state:
            ...
            logger.debug(f"Waiting for DynamoDB table {self.table_name} to become {expected}.")
