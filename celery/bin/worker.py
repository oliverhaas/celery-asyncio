...
from celery.bin.base import COMMA_SEPARATED_LIST, LOG_LEVEL, CeleryDaemonCommand, CeleryOption, handle_preload_options
...
@click.option(
    ...
    type=int,
    cls=CeleryOption,
    callback=lambda ctx, _, value: value or ctx.obj.app.conf.worker_loop_workers,
    help_group="Pool Options",
    help="Number of event loop threads for async tasks.",
)
@click.option(
    "--loop-concurrency",
    type=int,
    cls=CeleryOption,
    callback=lambda ctx, _, value: value or ctx.obj.app.conf.worker_loop_concurrency,
    help_group="Pool Options",
    help="Max concurrent async tasks per event loop thread.",
)
@click.option(
    "--sync-workers",
    type=int,
    cls=CeleryOption,
    callback=lambda ctx, _, value: value or ctx.obj.app.conf.worker_sync_workers,
    help_group="Pool Options",
    help="Number of threads for sync tasks.",
)
@click.option(
    "-E",
    "--task-events",
