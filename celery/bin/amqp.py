    ...
    if amqp_context.channel is None:
        ...
        try:
            amqp_context.channel.exchange_declare(
                exchange=exchange, type=type, passive=passive, durable=durable, auto_delete=auto_delete
            )
...
def queue_bind(amqp_context, queue, exchange, routing_key):
    if amqp_context.channel is None:
        ...
        try:
            amqp_context.channel.queue_bind(queue=queue, exchange=exchange, routing_key=routing_key)
        except Exception as e:
            amqp_context.echo_error(e)
            amqp_context.reconnect()
        else:
...
def queue_declare(amqp_context, queue, passive, durable, auto_delete):
    if amqp_context.channel is None:
        ...
        try:
            ...
            amqp_context.cli_context.secho("queue:{} messages:{} consumers:{}".format(*retval), fg="cyan", bold=True)
            amqp_context.echo_ok()
...
def queue_delete(amqp_context, queue, if_unused, if_empty):
    if amqp_context.channel is None:
        ...
        try:
            retval = amqp_context.channel.queue_delete(queue=queue, if_unused=if_unused, if_empty=if_empty)
        except Exception as e:
            amqp_context.echo_error(e)
            amqp_context.reconnect()
        else:
            amqp_context.cli_context.secho(f"{retval} messages deleted.", fg="cyan", bold=True)
            amqp_context.echo_ok()
    ...
    if amqp_context.channel is None:
        ...
        try:
            amqp_context.channel.basic_publish(
                msg, exchange=exchange, routing_key=routing_key, mandatory=mandatory, immediate=immediate
            )
        except Exception as e:
            amqp_context.echo_error(e)
            amqp_context.reconnect()
