...
def _backup(filename, suffix=".orig"):
    lines = []
    backup_filename = f"{filename}{suffix}"
    print(f"writing backup to {backup_filename}...", file=sys.stderr)
    with codecs.open(filename, "r", "utf-8") as read_fh, codecs.open(backup_filename, "w", "utf-8") as backup_fh:
        for line in read_fh:
...
def _to_new_key(line, keyfilter=pass1, source=defaults._TO_NEW_KEY):
    ...
    for old_key in sorted(source, key=len, reverse=True):
...
@click.option(
    "--compat", cls=CeleryOption, is_flag=True, help_group="Upgrading Options", help="Maintain backwards compatibility."
)
@click.option(
