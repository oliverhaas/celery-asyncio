...
class Task:
    ...
    def bind(cls, app):
        ...
        # Neither of the new flags was set anywhere, so inherit whatever the
        # one they replace says (upstream 40c234919).
        if cls.acks_on_failure is None:
            cls.acks_on_failure = cls.acks_on_failure_or_timeout if cls.acks_on_failure_or_timeout is not None else True
        if cls.acks_on_timeout is None:
            cls.acks_on_timeout = cls.acks_on_failure_or_timeout if cls.acks_on_failure_or_timeout is not None else True
