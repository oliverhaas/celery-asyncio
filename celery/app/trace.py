...
log_policy_reject = log_policy_t(LOG_REJECTED, "rejected", logging.WARNING, 1, 1)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    I, R, state, retval = await on_error(task_request, exc, RETRY, call_errbacks=False)
                    traceback_clear(exc)
                except Exception as exc:
                    I, R, state, retval = await on_error(task_request, exc)
                    traceback_clear(exc)
                except asyncio.CancelledError:
                    reason = async_cancellation_reason.get()
                    cancelled_as = reason() if reason is not None else None
                    if cancelled_as is None:
                        raise
                    I, R, state, retval = await on_error(task_request, cancelled_as)
                    traceback_clear(cancelled_as)
                except BaseException:
                    raise
                else:
                    try:
...
def report_internal_error(task, exc):
    ...
    try:
        ...
        warn(RuntimeWarning(f"Exception raised outside body: {exc!r}:\n{exc_info.traceback}"), stacklevel=2)
        return exc_info
    finally:
        del _tb
