...
class test_Serialization:
    ...
    def test_content_type_decoding(self):
        ...
        assert (
            loads(latin_string_as_latin1, content_type="application/data", content_encoding="latin-1") == latin_string
        )

    def test_content_type_binary(self):
        assert isinstance(
            loads(unicode_string_as_utf8, content_type="application/data", content_encoding="binary"),
            bytes,
        )
    ...
    def test_content_type_encoding(self):
        ...
        assert dumps(unicode_string, serializer="raw")[-1] == unicode_string_as_utf8
        assert dumps(latin_string, serializer="raw")[-1] == latin_string_as_utf8
    ...
    def test_reraises_DecodeError(self):
        with pytest.raises(DecodeError):
            loads(object(), content_type="application/json", content_encoding="utf-8")

    def test_json_loads(self):
        assert loads(json_data, content_type="application/json", content_encoding="utf-8") == py_data
    ...
    def test_yaml_loads(self):
        ...
        register_yaml()
        assert loads(yaml_data, content_type="application/x-yaml", content_encoding="utf-8") == py_data
    ...
    def test_pickle_loads(self):
        assert loads(pickle_data, content_type="application/x-python-serialize", content_encoding="binary") == py_data
    ...
    def test_loads__not_accepted(self):
        ...
        with pytest.raises(ContentDisallowed):
            loads("tainted", "application/x-evil", "binary", accept=["application/x-json"])
        assert loads("tainted", "application/x-doomsday", "binary", accept=["application/x-doomsday"])
