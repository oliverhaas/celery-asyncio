...
class test_secureserializer(SecurityCase):
    def _get_s(self, key, cert, certs, serializer="json"):
        ...
        return SecureSerializer(PrivateKey(key), Certificate(cert), store, serializer=serializer)

    @pytest.mark.parametrize("data", [1, "foo", b"foo", {"foo": 1}, {"foo": DEFAULT_SEPARATOR}])
