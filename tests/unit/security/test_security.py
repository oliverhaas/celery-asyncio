...
class test_security(SecurityCase):
    ...
    def test_setup_registry_complete(self, dis, reg, key="KEY", cert="CERT"):
        ...
        with conftest.open(side_effect=effect), patch("celery.security.registry") as registry:
            store = Mock()
    ...
    def test_reraise_errors(self):
        with pytest.raises(SecurityError), reraise_errors(errors=(KeyError,)):
            raise KeyError("foo")
        with pytest.raises(KeyError), reraise_errors(errors=(ValueError,)):
            raise KeyError("bar")
