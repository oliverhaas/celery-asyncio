...
if CredentialProvider is not None:

    class MyCredentialProvider(CredentialProvider):
        pass
else:
    MyCredentialProvider = None
...
class test_RedisBackend(basetest_RedisBackend):
    ...
    def test_timeouts_in_url_coerced(self):
        ...
        x = self.Backend(
            ("redis://:bosco@vandelay.com:123//1?socket_timeout=30&socket_connect_timeout=100"),
            app=self.app,
        )
        assert x.connparams
        assert x.connparams["host"] == "vandelay.com"
    ...
    @pytest.mark.parametrize(
        "uri",
        ["rediss://:bosco@vandelay.com:123//1?ssl_cert_reqs=CERT_KITTY_CATS", "rediss://:bosco@vandelay.com:123//1"],
    )
    ...
    def test_retry_policy_conf(self, retry_over_time):
        ...
        retry_over_time.assert_called_with(
            fn, b.connection_errors, (), {}, ANY, max_retries=2, interval_start=0, interval_step=0.01, interval_max=1
        )
