...
class test_AzureBlockBlobBackend:
    def setup_method(self):
        self.url = (
            "azureblockblob://DefaultEndpointsProtocol=protocol;AccountName=name;AccountKey=key;EndpointSuffix=suffix"
        )
    ...
    def test_configure_client(self, mock_blob_service_factory):
        ...
        (
            mock_blob_service_factory.from_connection_string.assert_called_once_with(
                connection_string, connection_timeout=connection_timeout, read_timeout=read_timeout
            )
        )
    ...
    def test_mget(self, mock_client, base_path):
        ...
        mock_client.get_blob_client.assert_has_calls(
            [call(blob=base_path + key.decode(), container="celery") for key in keys], any_order=True
        )
...
class test_as_uri:
    ...
    def test_as_uri_exclude_password(self):
        assert self.backend.as_uri(include_password=False) == (
            "azureblockblob://DefaultEndpointsProtocol=protocol;AccountName=name;AccountKey=**;EndpointSuffix=suffix"
        )
