...
from celery.exceptions import ExceptionInfo, ImproperlyConfigured
...
class test_ElasticsearchBackend:
    ...
    def test_index_conflict(self, datetime_mock):
        expected_dt = datetime(2020, 6, 1, 18, 43, 24, 123456, UTC)
    ...
    def test_backend_concurrent_update(self, base_datetime_mock, es_datetime_mock):
        ...
        expected_done_dt = datetime(2020, 6, 1, 18, 45, 34, 654321, UTC)
            ...
            x._server.index.assert_has_calls(
                [
                    call(
                        id=encoded_task_id,
                        index=x.index,
                        body={"result": expected_result, "@timestamp": expected_dt.isoformat()[:-9] + "Z"},
                        params={"op_type": "create"},
                    ),
                    call(
                        id=encoded_task_id,
            ...
            x._server.update.assert_has_calls(
                [
                    call(
                        id=encoded_task_id,
                        index=x.index,
                        body={"doc": {"result": expected_result, "@timestamp": expected_dt.isoformat()[:-9] + "Z"}},
                        params={"if_seq_no": 2, "if_primary_term": 1},
                    ),
                    call(
                        id=encoded_task_id,
    ...
    def test_index(self):
        ...
        x._server.index.assert_called_once_with(
            id=str(sentinel.task_id), index=x.index, body=body, params={"op_type": "create"}, kwarg1="test1"
        )
