...
class test_DynamoDBBackend:
    ...
    def test_get_client_explicit_endpoint(self):
        ...
        with patch("boto3.client") as mock_boto_client, patch(table_creation_path):
            ...
            mock_boto_client.assert_called_once_with(
                "dynamodb", endpoint_url="http://my.domain.com:666", region_name="us-east-1"
            )
    ...
    def test_get_client_local(self, dynamodb_host):
        table_creation_path = "celery.backends.dynamodb.DynamoDBBackend._get_or_create_table"
        with patch("boto3.client") as mock_boto_client, patch(table_creation_path):
            backend = DynamoDBBackend(app=self.app, url=f"dynamodb://@{dynamodb_host}:8000")
            client = backend._get_client()
            assert backend.client is client
            mock_boto_client.assert_called_once_with(
                "dynamodb", endpoint_url=f"http://{dynamodb_host}:8000", region_name="us-east-1"
            )
    ...
    def test_get_client_credentials(self):
        ...
        with patch("boto3.client") as mock_boto_client, patch(table_creation_path):
            ...
            mock_boto_client.assert_called_once_with(
                "dynamodb", aws_access_key_id="key", aws_secret_access_key="secret", region_name="test"
            )
    ...
    def test_set_table_ttl_enable_when_disabled_succeeds(self):
        ...
        mock_describe_time_to_live.return_value = {
            "TimeToLiveDescription": {"TimeToLiveStatus": "DISABLED", "AttributeName": self.backend._ttl_field.name}
        }
    ...
    def test_set_table_ttl_enable_when_enabled_with_correct_attr_succeeds(self):
        ...
        mock_describe_time_to_live = self.backend._client.describe_time_to_live = MagicMock()
        mock_describe_time_to_live.return_value = {
            "TimeToLiveDescription": {"TimeToLiveStatus": "ENABLED", "AttributeName": self.backend._ttl_field.name}
        }
    ...
    def test_set_table_ttl_enable_when_currently_disabling_raises(self):
        ...
        mock_describe_time_to_live = self.backend._client.describe_time_to_live = MagicMock()
        mock_describe_time_to_live.return_value = {
            "TimeToLiveDescription": {"TimeToLiveStatus": "DISABLING", "AttributeName": self.backend._ttl_field.name}
        }
        ...
        client_error = ClientError(
            {
                "Error": {
                    "Code": "ValidationException",
                    "Message": (
                        f"TimeToLive is active on a different AttributeName: current AttributeName is {wrong_attr_name}"
                    ),
                }
            },
            "UpdateTimeToLive",
    ...
    def test_set_table_ttl_disable_when_currently_enabling_raises(self):
        ...
        mock_describe_time_to_live = self.backend._client.describe_time_to_live = MagicMock()
        mock_describe_time_to_live.return_value = {
            "TimeToLiveDescription": {"TimeToLiveStatus": "ENABLING", "AttributeName": self.backend._ttl_field.name}
        }
    ...
    def test_item_to_dict(self):
        boto_response = {
            "Item": {"id": {"S": sentinel.key}, "result": {"B": sentinel.value}, "timestamp": {"N": Decimal(1)}}
        }
