...
class test_GCSBackend:
    ...
    def test_missing_storage_module(self):
        with patch("celery.backends.gcs.storage", None), pytest.raises(ImproperlyConfigured, match="You must install"):
            GCSBackend(app=self.app)

    def test_missing_firestore_module(self):
        with (
            patch("celery.backends.gcs.firestore", None),
            pytest.raises(ImproperlyConfigured, match="You must install"),
        ):
            GCSBackend(app=self.app)
    ...
    @patch.object(GCSBackend, "_is_firestore_ttl_policy_enabled")
    def test_bucket_ttl_missing_lifecycle_rule(self, mock_firestore_ttl, mock_lifecycle):
    ...
    def test_set_key(
        ...
        mock_blob.upload_from_string.assert_called_once_with("testvalue", retry=backend._retry_policy)
        if gcs_ttl:
    ...
    @patch("celery.backends.gcs.getpid")
    def test_new_firestore_client_after_fork(self, mock_pid, mock_firestore_client, mock_firestore_ttl):
    ...
    def test_is_firestore_ttl_policy_enabled(self, mock_get_field_request, mock_firestore_admin_client):
    ...
    def test_on_chord_part_return(
        ...
        mock_maybe_signature.return_value.delay.assert_called_once_with(["result1", "result2"])
    ...
    def test_firestore_document(self, mock_firestore_client, mock_firestore_ttl):
        ...
        mock_firestore_client.collection.assert_called_once_with(backend._collection_name)
