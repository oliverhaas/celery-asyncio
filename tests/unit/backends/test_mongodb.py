...
pytest.importorskip("dns.version")
    ...
    def mock_resolver(_, rdtype, rdclass=None, lifetime=None, **kwargs):
        ...
        if rdtype == "SRV":
            return [
                ...
                for hostname in ["mongo1.example.com", "mongo2.example.com", "mongo3.example.com"]
            ]
        elif rdtype == "TXT":
...
class test_MongoBackend:
    ...
    @pytest.mark.skipif(
        dns.version.MAJOR > 1, reason="For dnspython version > 1, pymongo'ssrv_resolver calls resolver.resolve"
    )
    ...
    @pytest.mark.skipif(
        dns.version.MAJOR <= 1, reason="For dnspython versions 1.X, pymongo'ssrv_resolver calls resolver.query"
    )
    @pytest.mark.skipif(pymongo.version_tuple[0] > 3, reason="For pymongo version > 3, options returns ssl")
    ...
    @pytest.mark.skipif(
        dns.version.MAJOR > 1, reason="For dnspython version >= 2, pymongo'ssrv_resolver calls resolver.resolve"
    )
    ...
    def test_get_connection_with_authmechanism_no_username(self):
        with patch("pymongo.MongoClient") as mock_Connection:
            ...
            if "authMechanism" in mb.options:  # pragma: no cover  # pymongo >= 4.14
                ...
                authmechanism_option = "authmechanism"
            mock_Connection.assert_called_once_with(
                host=["localhost:27017"], **{**mb._prepare_client_options(), authmechanism_option: "SCRAM-SHA-256"}
            )
    ...
    def test_get_task_meta_for_no_result(self, mock_get_database):
        ...
        mock_get_database.assert_called_once_with()
        mock_database.__getitem__.assert_called_once_with(MONGODB_COLLECTION)
        assert ret_val == {"status": states.PENDING, "result": None}
    ...
    def test_restore_group(self, mock_get_database):
        ...
        assert sorted(["date_done", "result", "task_id"]) == sorted(ret_val.keys())
    ...
    def test_regression_worker_startup_info(self):
        self.app.conf.result_backend = (
            "mongodb://user:password@host0.com:43437,host1.com:43437/work4us?replicaSet=rs&ssl=true"
        )
