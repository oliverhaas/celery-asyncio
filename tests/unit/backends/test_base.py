...
from celery.utils.serialization import UnpickleableExceptionWrapper, subclass_exception
...
class test_Backend_interface:
    ...
    def test_get_result_meta(self):
        ...
        request = Context(args=args, kwargs=kwargs, task=task_name, delivery_info={"routing_key": "celery"})
        meta = b2._get_result_meta(
            result={"fizz": "buzz"}, state=states.SUCCESS, traceback=None, request=request, encode=False
        )
    ...
    def test_get_result_meta_stamps_attribute_error(self):
        ...
        self.app.conf.result_extended = True
        b1 = BaseBackend(self.app)
        meta = b1._get_result_meta(result={"fizz": "buzz"}, state=states.SUCCESS, traceback=None, request=Request())
        assert meta["status"] == states.SUCCESS
        assert meta["result"] == {"fizz": "buzz"}
        assert meta["traceback"] is None
    ...
    def test_get_result_meta_encoded(self):
        ...
        meta = b1._get_result_meta(
            result={"fizz": "buzz"}, state=states.SUCCESS, traceback=None, request=request, encode=True
        )
    ...
    def test_get_result_meta_with_none(self):
        b1 = BaseBackend(self.app)
        meta = b1._get_result_meta(result=None, state=states.SUCCESS, traceback=None, request=None)
        assert meta["status"] == states.SUCCESS
        ...
        b2 = BaseBackend(self.app)
        request = Context(args=args, kwargs=kwargs, task=task_name, delivery_info={"routing_key": "celery"})
        meta = b2._get_result_meta(result=None, state=states.SUCCESS, traceback=None, request=request, encode=False)
        assert meta["name"] == task_name
        assert meta["args"] == args
        assert meta["kwargs"] == kwargs
        assert meta["queue"] == "celery"
    ...
    def test_get_result_meta_format_date(self):
        ...
        request = Context(args=args, kwargs=kwargs)
        meta = b1._get_result_meta(
            result={"fizz": "buzz"}, state=states.SUCCESS, traceback=None, request=request, format_date=True
        )
        ...
        request = Context(args=args, kwargs=kwargs)
        meta = b2._get_result_meta(
            result={"fizz": "buzz"}, state=states.SUCCESS, traceback=None, request=request, format_date=False
        )
...
class test_prepare_exception:
    ...
    def test_unicode_message(self):
        ...
        assert x == {"exc_message": (message,), "exc_type": Exception.__name__, "exc_module": Exception.__module__}
...
class test_BaseBackend_dict:
    ...
    def test_mark_as_failure__bound_errback_eager(self):
        ...
        group.return_value.apply.assert_called_with((request.id,), parent_id=request.id, root_id=request.root_id)
    ...
    def test_mark_as_failure__bound_errback(self):
        ...
        b.mark_as_failure("id", exc, request=request)
        group.assert_called_with(request.errbacks, app=self.app)
        group.return_value.apply_async.assert_called_with((request.id,), parent_id=request.id, root_id=request.root_id)
    ...
    def test_chord_error_from_stack_raises(self):
        ...
        with patch.object(b, "_call_task_errbacks", side_effect=ExpectedException()) as mock_call_errbacks:
    ...
    def test_not_an_exception_but_a_callable(self):
        ...
        with pytest.raises(
            SecurityError, match=re.escape(r"Expected an exception class, got os.system with payload ('echo 1',)")
        ):
    ...
    def test_not_an_exception_but_another_object(self):
        ...
        with pytest.raises(
            SecurityError, match=re.escape(r"Expected an exception class, got builtins.object with payload ()")
        ):
            self.b.exception_to_python(x)
    ...
    def test_exception_to_python_when_type_error(self):
        ...
        test_exception = {"exc_type": "TestParamException", "exc_module": "celery", "exc_message": []}
    ...
    def test_create_chord_error_with_cause(self, message, original_exc, expected_cause_behavior):
    ...
    def test_create_fake_task_request(self, task_id, errbacks, task_name, extra_kwargs, expected_attrs):
    ...
    def _create_mock_frozen_group(self, group_id="group-id", task_ids=None, task_names=None):
        ...
        results = []
        for task_id, task_name in zip(task_ids, task_names, strict=False):
            result = Mock()
            result.id = task_id
...
class test_backend_retries:
    ...
    def test_get_failed_never_retries(self):
        ...
        try:
            ...
            b._sleep = Mock()
            b._get_task_meta_for = Mock()
            b._get_task_meta_for.side_effect = [expected_exc, {"status": states.SUCCESS, "result": 42}]
            try:
    ...
    def test_get_with_retries(self):
        ...
        try:
            ...
            b._sleep = Mock()
            b._get_task_meta_for = Mock()
            b._get_task_meta_for.side_effect = [Exception("failed"), {"status": states.SUCCESS, "result": 42}]
