...
class test_S3Backend:
    ...
    def test_it_creates_an_aws_s3_connection(self, mock_boto3, aws_region):
        ...
        mock_boto3.Session.assert_called_once_with(
            aws_access_key_id="somekeyid", aws_secret_access_key="somesecret", region_name=aws_region
        )

    @pytest.mark.parametrize(
        "endpoint_url", [None, "https://custom.s3"], ids=["No given endpoint url", "Custom endpoint url"]
    )
    @patch("celery.backends.s3.boto3")
    ...
    def _mock_s3_resource(self):
        ...
        session = boto3.Session(
            aws_access_key_id="moto_key_id", aws_secret_access_key="moto_secret_key", region_name="us-east-1"
        )
