...
class test_multi_args:
    ...
    def test_parse(self, gethostname, mkdirs_mock):
        ...
        p.parse()
        it = multi_args(p, cmd="celery multi", append="*AP*", prefix="*P*", suffix="*S*")
        nodes = list(it)
        ...
        nodes2 = list(multi_args(p, cmd="celery multi", append="", prefix="*P*", suffix="*S*"))
        ...
        assert nodes3[0].argv == ("celery multi", "-c 5", "-n celery1@example.com") + _args("celery1")
        for i, worker in enumerate(nodes3[1:]):
            assert worker.name == "celery%s@example.com" % (i + 2)
            node_i = f"celery{i + 2}"
            assert worker.argv == ("celery multi", f"-n {node_i}@example.com") + _args(node_i)

        nodes4 = list(multi_args(p2, cmd="celery multi", suffix='""'))
        assert len(nodes4) == 10
        assert nodes4[0].name == "celery1@"
        assert nodes4[0].argv == ("celery multi", "-c 5", "-n celery1@") + _args("celery1")
        ...
        assert nodes7[0].argv == ("celery multi", "-Q test", "-n foo@bar") + _args("foo")
...
class test_Cluster:
    def setup_method(self):
        ...
        with patch("celery.apps.multi.os.mkdir"):
            self.cluster = Cluster(
                [Node("foo@example.com"), Node("bar@example.com"), Node("baz@example.com")],
    ...
    def test_send_all(self):
        ...
        self.cluster.on_node_signal.assert_has_calls(call(node, "TERM") for node in nodes)
        for node in nodes:
