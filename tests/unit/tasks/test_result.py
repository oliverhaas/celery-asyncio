...
class test_AsyncResult:
    ...
    def test_get_timeout(self):
        ...
        pending_res = self.app.AsyncResult(uuid())
        with patch("celery.result.time") as _time, pytest.raises(TimeoutError):
    ...
    def test_date_done(self, utc_datetime_mock, timezone, date):
        utc_datetime_mock.return_value = datetime.datetime(2024, 8, 24, 0, 0, 0, 0, datetime.UTC)
...
class test_GroupResult:
    ...
    def test_eq_with_parent(self):
        ...
        grp_res = self.app.GroupResult(
            uuid(), [self.app.AsyncResult(uuid()) for _ in range(10)], parent=self.app.AsyncResult(uuid())
        )
...
class test_tuples:
    ...
    def test_GroupResult_as_tuple(self):
        ...
        result = self.app.GroupResult(
            "group-result-1", [self.app.AsyncResult(f"async-result-{i}") for i in range(2)], parent
        )
