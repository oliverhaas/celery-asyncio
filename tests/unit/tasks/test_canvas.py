...
SIG = Signature(
    {"task": "TASK", "args": ("A1",), "kwargs": {"K1": "V1"}, "options": {"task_id": "TASK_ID"}, "subtask_type": ""},
)
...
def _group_result_sizes_in_as_tuple(tuple_repr):
    """Return the fan-out size of every GroupResult node in an as_tuple() tree."""
    sizes = []
...
def _group_result_sizes_on_spine(result):
    """Return the fan-out size of every GroupResult on the parent spine."""
    sizes = []
    node = result
...
class test_Signature(CanvasCase):
    ...
    def test_OR(self, subtests):
        ...
        with subtests.test("Test chaining with a non-signature object"), pytest.raises(TypeError):
...
class test_chain(CanvasCase):
    ...
    def test_empty_groups_are_skipped_in_chain(self):
        # An empty group upgraded to a chord gives it a header that never
        # completes, so everything behind it stalls.
        c = chain(
    ...
    def test_empty_group_body_is_skipped_when_chain_upgrades_to_chord(self):
        c = self.add.s(1, 1) | group([self.add.s(2, 2), self.add.s(3, 3)], app=self.app) | group(app=self.app)
    ...
    def test_generator_backed_empty_group_is_not_skipped_when_chained(self):
        # Asking whether a generator-backed group is empty would consume it,
        # so it is left alone and still upgrades to a chord.
        def tasks():
            yield from ()
    ...
    def test_group_to_chord__protocol_2__or(self):
        c = group([self.add.s(i, i) for i in range(5)], app=self.app) | self.add.s(10) | self.add.s(20) | self.add.s(30)
        assert isinstance(c, chord)

    def test_group_to_chord__protocol_2(self):
        c = chain(
            group([self.add.s(i, i) for i in range(5)], app=self.app), self.add.s(10), self.add.s(20), self.add.s(30)
        )
        assert isinstance(c, chord)
    ...
    def test_chord_to_chain(self):
        c = chord([self.add.s("x0", "y0"), self.add.s("x1", "y1")], self.add.s(["foo"])) | chain(
            self.add.s(["y"]), self.add.s(["z"])
        )
        assert isinstance(c, _chain)
        assert c.apply().get() == ["x0y0", "x1y1", "foo", "y", "z"]

    def test_chord_to_group(self):
        c = chord([self.add.s("x0", "y0"), self.add.s("x1", "y1")], self.add.s(["foo"])) | group(
            [self.add.s(["y"]), self.add.s(["z"])]
        )
        assert isinstance(c, _chain)
        assert c.apply().get() == [["x0y0", "x1y1", "foo", "y"], ["x0y0", "x1y1", "foo", "z"]]
    ...
    def test_chain_of_chord_upgrade_on_chaining__protocol_2(self, group_last_task):
        c = chain(
            ...
            self.add.s(30) if not group_last_task else group(self.add.s(30), app=self.app),
        )
        assert isinstance(c, _chain)
        assert len(c.tasks) == 1, "Consecutive chords should be further upgraded to a single chord."
    ...
    def test_chord_sets_result_parent(self):
        g = self.add.s(0, 0) | group(self.add.s(i, i) for i in range(1, 10)) | self.add.s(2, 2) | self.add.s(4, 4)
        res = g.freeze()
    ...
    def test_group_in_center_of_chain(self):
        t1 = chain(
            self.add.si(1, 1), group(self.add.si(1, 1), self.add.si(1, 1)), self.add.si(1, 1) | self.add.si(1, 1)
        )
    ...
    def test_consecutive_groups_in_chain_preserve_group_results_in_as_tuple(self, task_protocol):
        # Regression for #8903: chain(head, ..., group(G1), group(G2), tail...)
        # must keep the GroupResult fan-out in as_tuple() rather than collapse
        # to a short parent spine with no group children. Both protocols,
        # because protocol 1 enables use_link in prepare_steps().
        self.app.conf.task_protocol = task_protocol
        n = 3
        worker_tasks = [self.add.si(i, i) for i in range(n)]
        post_tasks = [chain(self.add.si(i, 0), self.add.si(0, i), app=self.app) for i in range(n)]
        canvas = chain(
            self.add.si(0, 0),
            self.add.si(1, 0),
            self.add.si(0, 1),
        ...
        for sizes in (_group_result_sizes_in_as_tuple(tup), _group_result_sizes_on_spine(restored)):
...
class test_group(CanvasCase):
    ...
    def test_from_dict_deep_deserialize(self, group_type):
        ...
        assert all(isinstance(child_task, Signature) for child_task in deserialized_group.tasks)
    ...
    def test_from_dict_deeper_deserialize(self, group_type):
        ...
        deserialized_group = group_type.from_dict(serialized_group)
        assert isinstance(deserialized_group, group_type)
        assert all(isinstance(child_task, group_type) for child_task in deserialized_group.tasks)
        assert all(
    ...
    def test_apply_contains_chords_containing_chain_with_empty_tail(self):
        ...
        tail_count = 24
        gchild_sig = chain(
            (ggchild_sig,) * ggchild_count + (group((ggchild_sig,) * tail_count), group(())),
        )
        child_chord = chord((gchild_sig,), ggchild_sig)
        group_sig = group((child_chord,))
        with (
    ...
    def test_apply_contains_chords_containing_empty_chord(self):
        gchild_sig = chord((), self.add.si(0, 0))
        child_count = 24
        child_chord = chord((gchild_sig,), self.add.si(0, 0))
        group_sig = group((child_chord,) * child_count)
        with (
...
class test_chord(CanvasCase):
    ...
    def test_freeze_empty_group_body_returns_result(self):
        # An empty group body is falsy but still present, so freeze() has to
        # produce its GroupResult -- callers traversing chord body results
        # would otherwise see nothing where a body exists. Defensive, for
        # chains that upgrade a group into a chord with an empty group body.
        x = chord(
            group(self.add.s(2, 2), app=self.app),
            group(app=self.app),
            app=self.app,
    ...
    def test_chord_size_nested_group_chain_group_head_many(self):
        x = chord(group([group([self.add.s()] * 4) | self.add.s()] * 2), body=self.add.s())
        assert x.__length_hint__() == 2

    def test_chord_size_nested_group_chain_group_mid_single(self):
        x = chord(group([self.add.s() | group(self.add.s()) | self.add.s()] * 42), body=self.add.s())
        assert x.__length_hint__() == 42

    def test_chord_size_nested_group_chain_group_mid_many(self):
        x = chord(group([self.add.s() | group([self.add.s()] * 4) | self.add.s()] * 2), body=self.add.s())
        assert x.__length_hint__() == 2
    ...
    def test_chord_size_nested_group_chain_group_tail_many(self):
        x = chord(group([self.add.s() | group([self.add.s()] * 4)] * 2), body=self.add.s())
    ...
    def test_chord_size_deserialized_element_single(self):
        ...
        with patch("celery.canvas.Signature.from_dict", return_value=child_sig) as mock_from_dict:
    ...
    def test_from_dict_deep_deserialize(self, subtests):
        ...
        with subtests.test(msg="Validate chord header tasks is deserialized"):
            assert all(isinstance(child_task, Signature) for child_task in deserialized_chord.tasks)
    ...
    def test_from_dict_deep_deserialize_group(self, subtests, group_type):
        ...
        with subtests.test(msg="Validate chord header tasks are deserialized and unpacked"):
            assert all(
                isinstance(child_task, Signature) and not isinstance(child_task, group_type)
        ...
        with subtests.test(msg="Validate chord body is deserialized and not unpacked"):
            ...
            assert all(isinstance(body_child_task, Signature) for body_child_task in deserialized_chord.body.tasks)
    ...
    def test_from_dict_deeper_deserialize_group(self, subtests, group_type):
        ...
        # A header which is a group gets unpacked into the chord's `tasks`
        with subtests.test(msg="Validate chord header tasks are deserialized and unpacked"):
            assert all(isinstance(child_task, group_type) for child_task in deserialized_chord.tasks)
            assert all(
                isinstance(grandchild_task, Signature)
                for child_task in deserialized_chord.tasks
                for grandchild_task in child_task.tasks
        ...
        with subtests.test(msg="Validate chord body is deserialized and not unpacked"):
            ...
            assert all(isinstance(body_child_task, group) for body_child_task in deserialized_chord.body.tasks)
            assert all(
    ...
    def test_flag_allow_error_cb_on_chord_header_various_header_types(self):
        ...
        self.app.conf.task_allow_error_cb_on_chord_header = True
        headers = [signature("t"), [signature("t"), signature("t")], group(signature("t"), signature("t"))]
    ...
    @pytest.mark.parametrize("header", [[signature("s1"), signature("s2")], group(signature("s1"), signature("s2"))])
    @pytest.mark.usefixtures("depends_on_current_app")
    ...
    def test_chord_run_ensures_body_has_valid_task_id(self):
        ...
        with patch.object(body, "freeze", wraps=body.freeze) as mock_freeze:
            ...
            mock_freeze.assert_called_once_with(chord_task_id, group_id=group_task_id, root_id=None)
...
class test_merge_dictionaries(CanvasCase):
    def test_docstring_example(self):
        ...
        assert d1 == {"dict": {"a": 1, "b": 2}, "list": [1, 2, 3, 4], "tuple": (1, 2), "set": {"a", "b"}}
