...
class test_DelayedDelivery:
    ...
    def test_start_native_delayed_delivery_direct_exchange(self, caplog):
        ...
        consumer_mock.app.amqp.queues = {"celery": Queue("celery", exchange=Exchange("celery", type="direct"))}
    ...
    def test_start_native_delayed_delivery_topic_exchange(self, caplog):
        ...
        consumer_mock.app.conf.broker_native_delayed_delivery_queue_type = "classic"
        consumer_mock.app.conf.broker_url = "amqp://"
        consumer_mock.app.amqp.queues = {"celery": Queue("celery", exchange=Exchange("celery", type="topic"))}
    ...
    def test_start_native_delayed_delivery_fanout_exchange(self, caplog):
        ...
        consumer_mock.app.conf.broker_native_delayed_delivery_queue_type = "classic"
        consumer_mock.app.conf.broker_url = "amqp://"
        consumer_mock.app.amqp.queues = {"celery": Queue("celery", exchange=Exchange("celery", type="fanout"))}
    ...
    def test_on_retry_argument_types(self):
        ...
        with patch.object(delayed_delivery_instance, "_setup_delayed_delivery", side_effect=fake_exception):
            ...
            with patch.object(
                delayed_delivery_instance, "_on_retry", new=type_checking_errback.__get__(delayed_delivery_instance)
            ):
                try:
    ...
    def test_retry_over_time_with_float_return(self):
        ...
        with (
            patch.object(
                delayed_delivery, "_setup_delayed_delivery", side_effect=ConnectionRefusedError("Simulated failure")
            ),
    ...
    def test_bind_queues_raises_exceptions_on_failures(self, mock_bind):
        ...
        assert str(exception_group) == (
            "One or more failures occurred while binding queues to delayed delivery exchanges (2 sub-exceptions)"
        )

        # Verify the ExceptionGroup contains both exceptions
        exceptions = exception_group.exceptions
        assert len(exceptions) == 2
        assert any(isinstance(ex, ValueError) and str(ex) == "Queue1 binding failed" for ex in exceptions)
        assert any(isinstance(ex, RuntimeError) and str(ex) == "Queue3 binding failed" for ex in exceptions)
    ...
    def test_bind_retries_on_retried_exception(self, mock_bind, caplog):
        ...
        # occurred)
        assert bind_attempts[0] == 3, f"Expected 3 bind attempts (2 failures + 1 success), got {bind_attempts[0]}"

        # Verify retry warnings were logged
        warning_logs = [r for r in caplog.records if r.levelname == "WARNING"]
        retry_warnings = [r for r in warning_logs if "Retrying delayed delivery setup" in r.message]

        # Should have 2 retry warnings (one for each failed attempt)
        assert len(retry_warnings) == 2, (
            f"Expected 2 retry warnings, got {len(retry_warnings)}. All warnings: {[r.message for r in warning_logs]}"
        )
