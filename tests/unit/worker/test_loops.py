...
try:
    ...
    pytestmark = pytest.mark.skip(reason="kombu.asynchronous not available (kombu-asyncio rewrite)")
    Hub = None
...
if Hub is not None:
    ...
    # Provide fallback values so parametrize decorators don't fail at collection time
    EX_OK = 0
    EX_FAILURE = 1
    CLOSE = None
    RUN = None
...
class PromiseEqual:
    ...
    def __repr__(self):
        return f"<promise: {self.fun!r} {self.args!r} {self.kwargs!r}>"
    ...
    def close_then_error(self, mock=None, mod=0, exc=None):
        ...
        def first(*args, **kwargs):
            if not mod or mock.call_count > mod:
                self.close()
                raise (OSError() if exc is None else exc)
