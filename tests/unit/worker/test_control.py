...
class test_ControlPanel:
    ...
    def test_time_limit(self):
        ...
        r = panel.handle("time_limit", arguments={"task_name": self.mytask.name, "hard": 30, "soft": 10})
        assert self.mytask.time_limit == 30
        assert self.mytask.soft_time_limit == 10
        assert "ok" in r
        r = panel.handle("time_limit", arguments={"task_name": self.mytask.name, "hard": None, "soft": None})
        assert self.mytask.time_limit is None
        assert self.mytask.soft_time_limit is None
        assert "ok" in r

        r = panel.handle("time_limit", arguments={"task_name": "248e8afya9s8dh921eh928", "hard": 30})
        assert "error" in r
    ...
    def test_rate_limit_invalid_rate_limit_string(self):
        e = self.panel.handle("rate_limit", arguments={"task_name": "tasks.add", "rate_limit": "x1240301#%!"})
    ...
    def test_rate_limit(self):
        ...
        panel.handle("rate_limit", arguments={"task_name": task.name, "rate_limit": "100/m"})
    ...
    def test_rate_limit_nonexistant_task(self):
        self.panel.handle("rate_limit", arguments={"task_name": "xxxx.does.not.exist", "rate_limit": "1000/s"})
