...
class RequestCase:
    ...
    def xRequest(
        self, name=None, id=None, args=None, kwargs=None, on_ack=None, on_reject=None, Request=Request, **head
    ):
...
class test_Request(RequestCase):
    ...
    def test_terminate__pool_ref(self):
        ...
        with self.assert_signal_called(
            task_revoked, sender=job.task, request=job._context, terminated=True, expired=False, signum=signum
        ):
    ...
    def test_revoked_expires_expired(self):
        ...
        with self.assert_signal_called(
            task_revoked, sender=job.task, request=job._context, terminated=False, expired=True, signum=None
        ):
            job.revoked()
    ...
    def test_revoked(self):
        job = self.xRequest()
        with self.assert_signal_called(
            task_revoked, sender=job.task, request=job._context, terminated=False, expired=False, signum=None
        ):
            revoked.add(job.id)
            assert job.revoked()
    ...
    def test_fast_trace_task(self):
        ...
        try:
            ...
            failed, res, runtime = fast_trace_task(
                self.mytask.name, tid, message.headers, message.body, message.content_type, message.content_encoding
            )
            assert not failed
            assert res == repr(4**4)
...
class test_create_request_class(RequestCase):
    ...
    def test_execute_using_pool(self):
        ...
        self.pool.apply_async.assert_called_with(
            trace_task_ret,
            args=(job.type, job.id, job.request_dict, job.body, job.content_type, job.content_encoding),
