...
class test_solar:
    ...
    def test_is_due(self):
        ...
        assert self.s.is_due(datetime.now(UTC)).is_due

    def test_is_due__not_due(self):
        self.s.remaining_estimate = Mock(name="rem")
        self.s.remaining_estimate.return_value = timedelta(hours=10)
        assert not self.s.is_due(datetime.now(UTC)).is_due

    def test_remaining_estimate(self):
        self.s.cal = Mock(name="cal")
        self.s.cal.next_rising().datetime.return_value = datetime.now(UTC)
        self.s.remaining_estimate(datetime.now(UTC))
...
class test_crontab_parser:
    ...
    def test_eq(self):
        ...
        ) == self.crontab(minute="1", hour="2", day_of_week="5", day_of_month="10", month_of_year="5")
...
class test_crontab_remaining_estimate:
    ...
    def test_weekmonthdayyear(self):
        next = self.next_occurrence(
            self.crontab(minute=30, hour=14, day_of_week="fri", day_of_month=29, month_of_year=1),
            datetime(2010, 1, 22, 14, 30, 15),
        )
    ...
    def test_monthdayyear_not_week(self):
        next = self.next_occurrence(
            self.crontab(minute=[5, 42], day_of_week="wed,thu", day_of_month=29, month_of_year="1,4,7"),
            datetime(2010, 1, 29, 14, 30, 15),
        )
    ...
    def test_weekdaymonthyear_not_monthday(self):
        next = self.next_occurrence(
            self.crontab(minute=30, hour=14, day_of_week="fri", day_of_month=29, month_of_year="1-10"),
            datetime(2010, 1, 29, 14, 30, 15),
        )
    ...
    def test_weekmonthday_not_monthyear(self):
        next = self.next_occurrence(
            self.crontab(minute=[5, 42], day_of_week="fri", day_of_month=29, month_of_year="2-10"),
            datetime(2010, 1, 29, 14, 30, 15),
        )
    ...
    def test_weekday_not_monthdayyear(self):
        next = self.next_occurrence(
            self.crontab(minute=[5, 42], day_of_week="mon", day_of_month=18, month_of_year="2-10"),
            datetime(2010, 1, 11, 0, 5, 15),
        )
    ...
    def test_monthday_not_weekdaymonthyear(self):
        next = self.next_occurrence(
            self.crontab(minute=[5, 42], day_of_week="mon", day_of_month=29, month_of_year="2-4"),
            datetime(2010, 1, 29, 0, 5, 15),
        )
    ...
    def test_not_weekmonthdayyear(self):
        next = self.next_occurrence(
            self.crontab(minute=[5, 42], day_of_week="fri,sat", day_of_month=29, month_of_year="2-10"),
...
class test_crontab_is_due:
    ...
    @pytest.mark.parametrize(
        "minute,expected",
        [(30, {30}), ("30", {30}), ((30, 40, 50), {30, 40, 50}), ((30, 40, 50, 51), {30, 40, 50, 51})],
    )
