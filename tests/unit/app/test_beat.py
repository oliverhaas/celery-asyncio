...
class mScheduler(beat.Scheduler):
    ...
    def send_task(self, name=None, args=None, kwargs=None, **options):
        self.sent.append({"name": name, "args": args, "kwargs": kwargs, "options": options})
...
class test_Scheduler:
    ...
    def test_apply_entry_handles_empty_result(self):
        ...
        with patch.object(s, "apply_async") as mock_apply_async, patch("celery.beat.debug") as mock_debug:
    ...
    def test_due_tick(self):
        scheduler = mScheduler(app=self.app)
        scheduler.add(name="test_due_tick", schedule=always_due, args=(1, 2), kwargs={"foo": "bar"})
    ...
    def test_pending_left_10_milliseconds_tick(self):
        scheduler = mScheduler(app=self.app)
        scheduler.add(name="test_pending_left_10_milliseconds_tick", schedule=always_pending_left_10_milliseconds)
        assert scheduler.tick() == 0.010 - 0.010

    def test_honors_max_interval(self):
        scheduler = mScheduler(app=self.app)
        maxi = scheduler.max_interval
        scheduler.add(name="test_honors_max_interval", schedule=mocked_schedule(False, maxi * 4))
    ...
    def test_merge_inplace(self):
        ...
        a.update_from_dict(
            {"foo": {"schedule": mocked_schedule(True, 10)}, "bar": {"schedule": mocked_schedule(True, 20)}}
        )
        b.update_from_dict(
            {"bar": {"schedule": mocked_schedule(True, 40)}, "baz": {"schedule": mocked_schedule(True, 10)}}
        )
    ...
    def test_when(self):
        ...
        scheduler = mScheduler(app=self.app)
        result_utc = scheduler._when(mocked_schedule(True, 10, lambda: now_time_utc), 10)
        result_casey = scheduler._when(mocked_schedule(True, 10, lambda: now_time_casey), 10)
    ...
    def create_schedule_entry(self, schedule=None, args=(), kwargs=None, options=None, task=None):
        if options is None:
            options = {}
        if kwargs is None:
            kwargs = {}
...
def create_persistent_scheduler_w_call_logging(shelv=None):
    ...
    class MockPersistentScheduler(beat.PersistentScheduler):
        ...
        def send_task(self, task=None, args=None, kwargs=None, **options):
            self.sent.append({"task": task, "args": args, "kwargs": kwargs, "options": options})
            return self.app.AsyncResult(uuid())
...
class test_PersistentScheduler:
    ...
    def test_run_all_due_tasks_after_restart(self):
        ...
        app_schedule = {
            "first_missed": {"schedule": crontab(minute="*/10", nowfun=now_func), "task": "first_missed"},
            "second_missed": {"schedule": crontab(minute="*/1", nowfun=now_func), "task": "second_missed"},
            "non_missed": {"schedule": crontab(minute="*/13", nowfun=now_func), "task": "non_missed"},
        }
        shelve["entries"] = {
        ...
        for i in range(max_iter_number):
            ...
            if delay > 0:
                break
        assert {"first_missed", "second_missed"} == {item["task"] for item in scheduler.sent}
...
class test_schedule:
    def test_maybe_make_aware(self):
        ...
        x.utc_enabled = True
        d = x.maybe_make_aware(datetime.now(UTC))
        assert d.tzinfo
        x.utc_enabled = False
        d2 = x.maybe_make_aware(datetime.now(UTC))
        assert d2.tzinfo
