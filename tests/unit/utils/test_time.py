...
class test_LocalTimezone:
    def test_daylight(self, patching):
        ...
        time.daylight = False
        x = LocalTimezone()
        assert timedelta(seconds=-3600) == x.STDOFFSET
        assert x.DSTOFFSET == x.STDOFFSET
        time.daylight = True
        time.altzone = 3600
        y = LocalTimezone()
        assert timedelta(seconds=-3600) == y.STDOFFSET
        assert timedelta(seconds=-3600) == y.DSTOFFSET
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("2011-11-04T00:05:23+04:00", datetime(2011, 11, 4, 0, 5, 23, tzinfo=_timezone(timedelta(seconds=14400)))),
    ],
)
...
def test_remaining():
    # Relative
    remaining(datetime.now(UTC), timedelta(hours=1), relative=True)
    ...
    """
    start = datetime(month=3, day=9, year=2019, hour=10, minute=0, tzinfo=eastern_tz)  # EST
    ...
    next_actual_time = datetime(day=11, month=3, year=2019, hour=10, minute=0, tzinfo=eastern_tz)  # EDT
    ...
    In 2019, DST starts on March 10
    """
    start = datetime(day=10, month=3, year=2019, hour=1, minute=30, tzinfo=eastern_tz)  # EST

    now = datetime(day=10, month=3, year=2019, hour=1, minute=30, tzinfo=eastern_tz)  # EST
    delta = ffwd(hour=3, year=2019, microsecond=0, minute=30, second=0, day=10, weeks=0, month=3)
    # `next_actual_time` is the next time to run (derived from delta)
    next_actual_time = datetime(day=10, month=3, year=2019, hour=3, minute=30, tzinfo=eastern_tz)  # EDT
    assert start.tzname() == "EST"
...
class test_timezone:
    ...
    def test_to_local(self):
        assert timezone.to_local(make_aware(datetime.now(UTC), timezone.utc))
        assert timezone.to_local(datetime.now(UTC))

    def test_to_local_fallback(self):
        assert timezone.to_local_fallback(make_aware(datetime.now(UTC), timezone.utc))
        assert timezone.to_local_fallback(datetime.now(UTC))
...
class test_make_aware:
    ...
    def test_maybe_make_aware(self):
        ...
        tz = ZoneInfo("US/Eastern")
        eastern = datetime.now(UTC).replace(tzinfo=tz)
...
class test_localize:
    def test_standard_tz(self):
        ...
        tz = tzz()
        assert localize(make_aware(datetime.now(UTC), tz), tz)
    ...
    def test_when_zoneinfo(self, datetime_ambiguous_mock):
        ...
        assert localize(make_aware(datetime.now(UTC), tz2), tz2)
...
class test_get_exponential_backoff_interval:
    ...
    def test_with_jitter(self):
        assert get_exponential_backoff_interval(factor=4, retries=3, maximum=100, full_jitter=True) == 4 * (2**3) - 1

    def test_without_jitter(self):
        assert get_exponential_backoff_interval(factor=4, retries=3, maximum=100, full_jitter=False) == 4 * (2**3)

    def test_bound_by_maximum(self):
        maximum_boundary = 100
        assert get_exponential_backoff_interval(factor=40, retries=3, maximum=maximum_boundary) == maximum_boundary
