...
class ev_worker_heartbeats(replay):
    def setup(self):
        self.events = [
            Event("worker-heartbeat", hostname="utest1", timestamp=time() - HEARTBEAT_EXPIRE_WINDOW * 2),
...
class ev_task_states(replay):
    def setup(self):
        ...
        self.events = [
            ...
            Event("task-retried", uuid=tid, exception="KeyError('bar')", traceback="line 2 at main", hostname="utest1"),
            Event("task-failed", uuid=tid, exception="KeyError('foo')", traceback="line 1 at main", hostname="utest1"),
            Event("task-succeeded", uuid=tid, result="4", runtime=0.1234, hostname="utest1"),
            Event("foo-bar"),
            Event(
                "task-received",
                uuid=tid2,
...
def QTEV(type, uuid, hostname, clock, name=None, timestamp=None):
    """Quick task event."""
    return Event(f"task-{type}", uuid=uuid, hostname=hostname, clock=clock, name=name, timestamp=timestamp or time())
...
class ev_snapshot(replay):
    def setup(self):
        ...
        for i in range(20):
            ...
            self.events.append(Event("task-received", name=type, uuid=uuid(), hostname=worker))
...
class test_Worker:
    ...
    def test_heartbeat_expires__Decimal(self):
        assert heartbeat_expires(Decimal("344313.37"), freq=60, expire_window=200) == 344433.37
...
class test_Task:
    ...
    def test_info(self):
        ...
        assert sorted(task._info_fields + ("received",)) == sorted(task.info(extra=("received",)))

        assert sorted(["args", "kwargs"]) == sorted(task.info(["args", "kwargs"]).keys())
