...
import tests.skip
from celery.contrib.rdb import Rdb, debugger, set_trace  # noqa: T100
...
class test_Rdb:
    ...
    def test_set_trace(self, _frame, debugger):
        assert set_trace(Mock())  # noqa: T100
        assert set_trace()  # noqa: T100
    ...
    def test_rdb(self, get_avail_port):
        ...
        with Rdb(out=out) as rdb:
            ...
            # set_trace
            with patch("celery.contrib.rdb.Pdb.set_trace") as pset, patch("celery.contrib.rdb._frame"):
                rdb.set_trace()
                rdb.set_trace(Mock())
    ...
    def test_get_avail_port(self, sock):
        ...
        with pytest.raises(SockErr), Rdb(out=out):
            pass
        err.errno = errno.EADDRINUSE
        with pytest.raises(Exception), Rdb(out=out):
            pass
        called = [0]
