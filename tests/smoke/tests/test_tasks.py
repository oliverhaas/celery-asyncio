...
from tests.smoke.conftest import SuiteOperations, TaskTermination
from tests.smoke.tasks import (
    replace_with_task,
    ...
    def test_child_process_respawn(
        ...
        def wait_for_two_celery_processes():
            ...
            if len(pinfo_current) != 2:
                raise AssertionError(f"Child process did not respawn with method: {method.name}")
