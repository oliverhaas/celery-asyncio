...
from tests.integration.tasks import StampOnReplace, identity, replace_with_stamped_task
...
class test_stamping_hybrid_worker_cluster:
    ...
    def test_stamping_on_replace_with_legacy_worker_in_cluster(
        ...
        replaced_sig1 = identity.si(4).set(queue=legacy_worker.worker_queue).stamp(stamp1=stamp1["stamp1"])
        replaced_sig2 = identity.si(2).set(queue=legacy_worker.worker_queue).stamp(stamp2=stamp2["stamp2"])

        stamped_task = chain(
            replace_with_stamped_task.si(replace_with=replaced_sig1).set(queue=dev_worker.worker_queue),
            replace_with_stamped_task.si(replace_with=replaced_sig2).set(queue=dev_worker.worker_queue),
        )
        stamped_task.stamp(stamp=stamp["stamp"])
        stamped_task.apply_async().get(timeout=RESULT_TIMEOUT)
