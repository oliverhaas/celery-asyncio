...
from tests.integration.tasks import StampOnReplace, identity
from tests.smoke.tests.stamping.tasks import wait_for_revoke
...
def canvas(
    ...
    return chain(
        ...
        wait_for_revoke.s(waitfor_worker_queue=dev_worker.worker_queue).set(queue=dev_worker.worker_queue),
    )
...
class test_revoke_by_stamped_headers:
    def test_revoke_by_stamped_headers_after_publish(
        ...
        result: AsyncResult = canvas.apply_async(queue=celery_latest_worker.worker_queue)
