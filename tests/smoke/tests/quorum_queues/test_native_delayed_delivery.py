...
from tests.smoke.tasks import add, noop

if TYPE_CHECKING:
    from tests.smoke.tests.quorum_queues.conftest import RabbitMQManagementBroker
...
class test_broker_configuration_quorum:
    ...
    def test_native_delayed_delivery_queue_configuration(self, queues: list, celery_setup: CeleryTestSetup):
...
class test_native_delayed_delivery:
    ...
    def test_eta(self, celery_setup: CeleryTestSetup):
        ...
        result = s.apply_async(eta=datetime.now(UTC) + timedelta(0, 5))
    ...
    def test_eta_str(self, celery_setup: CeleryTestSetup):
        ...
        result = s.apply_async(eta=(datetime.now(UTC) + timedelta(0, 5)).isoformat())
    ...
    def test_long_delay(self, celery_setup: CeleryTestSetup, queues: list):
        ...
        assert result.status == "PENDING", f"Task should be PENDING but was {result.status}"
    ...
    def test_multiple_tasks_different_delays(self, celery_setup: CeleryTestSetup):
        ...
        results = [s.apply_async(eta=now + timedelta(seconds=delay)) for delay in (2, 4, 6)]
    ...
    def test_chain_with_delays(self, celery_setup: CeleryTestSetup):
        ...
        c = chain(add.s(1, 2).set(countdown=2), add.s(3).set(countdown=2), add.s(4).set(countdown=2)).set(
