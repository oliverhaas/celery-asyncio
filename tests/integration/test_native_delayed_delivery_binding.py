...
def get_bindings_for_exchange(exchange_name, vhost="/"):
    ...
    api_url = f"{get_management_api_url()}/exchanges/{vhost_encoded}/{exchange_encoded}/bindings/source"
...
def get_bindings_for_queue(queue_name, vhost="/"):
    ...
    api_url = f"{get_management_api_url()}/queues/{vhost_encoded}/{queue_encoded}/bindings"
    response = requests.get(api_url, auth=HTTPBasicAuth(user, password))
    response.raise_for_status()
    return response.json()
...
def test_worker_binds_consumed_queue_despite_earlier_queue_failure():
    ...
    with start_worker(
        ...
        queue_b_delayed_binding = [
            ...
            if b.get("destination") == exchange_name and b.get("routing_key") == f"#.{queue_b_name}"
        ]
        assert len(queue_b_delayed_binding) >= 1, (
            f"Expected delayed delivery binding for {queue_b_name!r}, but got bindings: {delayed_delivery_bindings!r}"
        )
        ...
        topic_to_queue_bindings = [b for b in queue_b_bindings if b.get("source") == exchange_name]
        topic_to_queue_routing_keys = {b.get("routing_key") for b in topic_to_queue_bindings}

        # Check the routing key for immediate delivery
        assert queue_b_name in topic_to_queue_routing_keys, (
            f"Expected routing key {queue_b_name!r} in bindings, but got: {topic_to_queue_bindings!r}"
        )
