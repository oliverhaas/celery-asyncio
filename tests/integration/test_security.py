...
class test_security:
    ...
    def gen_certificate(self, key, common_name, issuer=None, sign_key=None):
        ...
        certificate = (
            ...
            .issuer_name(x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, issuer or common_name)]))
            .not_valid_before(now)
            .not_valid_after(now + datetime.timedelta(seconds=86400))
            .serial_number(x509.random_serial_number())
            .public_key(key.public_key())
            .add_extension(x509.BasicConstraints(ca=True, path_length=0), critical=True)
            .sign(private_key=sign_key or key, algorithm=hashes.SHA256(), backend=default_backend())
        )
        return certificate
