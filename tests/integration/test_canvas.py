...
def await_redis_echo(expected_msgs, redis_key="redis-echo", timeout=TIMEOUT):
    ...
    while +expected_msgs:
        ...
        if maybe_key_msg is None:
            raise TimeoutError(f"Fetching from {redis_key!r} timed out - still awaiting {dict(+expected_msgs)!r}")
...
class test_link_error:
    ...
    def test_link_error_callback_retries(self, manager):
        exception = ExpectedException("Task expected to fail", "test")
        result = fail.apply_async(args=("test",), link_error=retry_once.s(countdown=None))
    ...
    def test_link_error_using_signature(self, manager):
        ...
        exception = ExpectedException("Task expected to fail", "test")
        assert (fail.delay().get(timeout=TIMEOUT / 10, propagate=False), True) == (exception, True)
...
class test_chain:
    ...
    def test_chain_replaced_with_a_chain_and_an_error_callback(self, manager):
        ...
        c = chain(
            identity.s("Hello "), replace_with_chain_which_raises.s(link_msg=link_msg), add.s(" will never be seen :(")
        )
        res = c.delay()
    ...
    def test_chain_with_eb_replaced_with_chain_with_eb(self, manager, subtests):
        ...
        with subtests.test(msg="Chain fails due to a child task dying"), pytest.raises(ValueError):
            res.get(timeout=TIMEOUT)
    ...
    def test_replace_chain_with_empty_chain(self, manager):
        ...
        with pytest.raises(ImproperlyConfigured, match="Cannot replace with an empty chain"):
    ...
    def test_chaining_upgraded_chords_mixed_canvas_protocol_2(self, manager, subtests, group_last_task):
        ...
        c = chain(
            group([redis_echo.si("1", redis_key=redis_key), redis_echo.si("2", redis_key=redis_key)]),
            group(
                [
                    redis_echo.si("3", redis_key=redis_key),
                    redis_echo.si("4", redis_key=redis_key),
...
class test_chord:
    ...
    def test_simple_chord_with_a_delay_in_group_save(self, manager, monkeypatch):
        ...
        monkeypatch.setattr(BaseKeyValueStoreBackend, "_apply_chord_incr", apply_chord_incr_with_sleep)
    ...
    def test_redis_subscribed_channels_leak(self, manager):
        ...
        total_chords = 10
        async_results = [chord([add.s(5, 6), add.s(6, 7)])(delayed_sum.s()) for _ in range(total_chords)]
        ...
        assert channels_before_count <= chord_header_task_count * total_chords + initial_channels_count
    ...
    def test_nested_chord(self, manager):
        ...
        c1 = chord(
            [chord([add.s(1, 2), add.s(3, 4)], add.s([5])), chord([add.s(6, 7)], add.s([10]))], add_to_all.s(["A"])
        )
        res1 = c1()
        ...
        c = group([group([group([group([add.s(1, 2)]) | add.s([3])]) | add.s([4])]) | add.s([5])]) | add.s([6])

        res = c()
        assert res.get(timeout=TIMEOUT) == [[[[3, 3], 4], 5], 6]
    ...
    def test_parent_ids__OR(self, manager):
        ...
        root = ids.si(i=1)
        expected_root_id = root.freeze().id
        g = root | ids.si(i=2) | group(ids.si(i=i) for i in range(3, 50)) | collect_ids.s(i=50) | ids.si(i=51)
        self.assert_parentids_chord(g(), expected_root_id)
    ...
    def test_chord_on_error(self, manager):
        ...
        c1 = chord(
            ...
            body=print_unicode.s("This should not be called").on_error(errback_old_style.s()),
        )
        res = c1()
        with pytest.raises(ExpectedException):
            res.get(propagate=True)
        ...
        redis_connection = get_redis_connection()
        # A hash keyed by task id, so a redelivered member overwrites its own
        # field rather than counting twice. group_index rides along in the
        # encoded payload as the last element, which is what restores the order.
        chord_results = sorted(
            (backend.decode(t) for t in redis_connection.hvals(j_key)),
            key=lambda cr: cr[4],
        )
    ...
    def test_chain_chord_chain_chord(self, manager):
        ...
        c = chain(
            identity.si(1),
            chord(
                [identity.si(2), chain(identity.si(3), chord([identity.si(4), identity.si(5)], identity.si(6)))],
                identity.si(7),
            ),
        )
        res = c.delay()
    ...
    @pytest.mark.parametrize(
        "errback_task",
        [errback_old_style, errback_new_style],
    )
    def test_mutable_errback_called_by_chord_from_simple(self, errback_task, manager, subtests):
        if not manager.app.conf.result_backend.startswith("redis"):
            raise pytest.skip("Requires redis result backend.")
        redis_connection = get_redis_connection()
    ...
    def test_error_propagates_to_chord_from_chain(self, manager, subtests):
        ...
        chord_sig = chord((child_sig,), identity.s())
        with subtests.test(msg="Error propagates from header chain which fails before the end"):
            res = chord_sig.delay()
            with pytest.raises(ExpectedException):
                res.get(timeout=TIMEOUT)

        chord_sig = chord((identity.si(42),), child_sig)
        with subtests.test(msg="Error propagates from body chain which fails before the end"):
            res = chord_sig.delay()
            with pytest.raises(ExpectedException):
                res.get(timeout=TIMEOUT)
    ...
    def test_immutable_errback_called_by_chord_from_chain(self, manager, subtests):
        ...
        with subtests.test(msg="Error propagates from header chain which fails before the end"):
            ...
            with pytest.raises(ExpectedException):
                res.get(timeout=TIMEOUT)
        with subtests.test(msg="Errback is called after header chain which fails before the end"):
            await_redis_echo({errback_msg}, redis_key=redis_key)
        ...
        with subtests.test(msg="Error propagates from body chain which fails before the end"):
            ...
            with pytest.raises(ExpectedException):
                res.get(timeout=TIMEOUT)
        with subtests.test(msg="Errback is called after body chain which fails before the end"):
            await_redis_echo({errback_msg}, redis_key=redis_key)
        redis_connection.delete(redis_key)
    ...
    def test_mutable_errback_called_by_chord_from_chain(self, errback_task, manager, subtests):
        if not manager.app.conf.result_backend.startswith("redis"):
            raise pytest.skip("Requires redis result backend.")
        redis_connection = get_redis_connection()
    ...
    def test_error_propagates_to_chord_from_chain_tail(self, manager, subtests):
        ...
        chord_sig = chord((child_sig,), identity.s())
        with subtests.test(msg="Error propagates from header chain which fails at the end"):
            res = chord_sig.delay()
            with pytest.raises(ExpectedException):
                res.get(timeout=TIMEOUT)

        chord_sig = chord((identity.si(42),), child_sig)
        with subtests.test(msg="Error propagates from body chain which fails at the end"):
            res = chord_sig.delay()
            with pytest.raises(ExpectedException):
                res.get(timeout=TIMEOUT)
    ...
    def test_immutable_errback_called_by_chord_from_chain_tail(self, manager, subtests):
        ...
        with subtests.test(msg="Error propagates from header chain which fails at the end"):
            ...
            with pytest.raises(ExpectedException):
                res.get(timeout=TIMEOUT)
        with subtests.test(msg="Errback is called after header chain which fails at the end"):
            await_redis_echo({errback_msg}, redis_key=redis_key)
        ...
        with subtests.test(msg="Error propagates from body chain which fails at the end"):
            ...
            with pytest.raises(ExpectedException):
                res.get(timeout=TIMEOUT)
        with subtests.test(msg="Errback is called after body chain which fails at the end"):
            await_redis_echo({errback_msg}, redis_key=redis_key)
        redis_connection.delete(redis_key)
    ...
    def test_mutable_errback_called_by_chord_from_chain_tail(self, errback_task, manager, subtests):
        if not manager.app.conf.result_backend.startswith("redis"):
            raise pytest.skip("Requires redis result backend.")
        redis_connection = get_redis_connection()
    ...
    def test_mutable_errback_called_by_chord_from_group(self, errback_task, manager, subtests):
        if not manager.app.conf.result_backend.startswith("redis"):
            raise pytest.skip("Requires redis result backend.")
        redis_connection = get_redis_connection()
    ...
    def test_immutable_errback_called_by_chord_from_group_fail_multiple(self, manager, subtests):
        if not manager.app.conf.result_backend.startswith("redis"):
            raise pytest.skip("Requires redis result backend.")
        redis_connection = get_redis_connection()
    ...
    def test_upgraded_chord_link_error_with_header_errback_enabled(self, manager, subtests):
        ...
        with subtests.test(msg="Error propagates from failure in header"), pytest.raises(ExpectedException):
    ...
    def test_chord_error_propagation_with_different_body_types(self, manager, caplog, input_body) -> None:
...
class test_signature_serialization:
    ...
    def test_rebuild_nested_chain_chain(self, manager):
        sig = chain(tasks.return_nested_signature_chain_chain.s(), tasks.rebuild_signature.s())
        sig.delay().get(timeout=TIMEOUT)

    def test_rebuild_nested_chain_group(self, manager):
        sig = chain(tasks.return_nested_signature_chain_group.s(), tasks.rebuild_signature.s())
        sig.delay().get(timeout=TIMEOUT)
    ...
    def test_rebuild_nested_chain_chord(self, manager):
        ...
        sig = chain(tasks.return_nested_signature_chain_chord.s(), tasks.rebuild_signature.s())
        sig.delay().get(timeout=TIMEOUT)

    def test_rebuild_nested_group_chain(self, manager):
        sig = chain(tasks.return_nested_signature_group_chain.s(), tasks.rebuild_signature.s())
        sig.delay().get(timeout=TIMEOUT)

    def test_rebuild_nested_group_group(self, manager):
        sig = chain(tasks.return_nested_signature_group_group.s(), tasks.rebuild_signature.s())
        sig.delay().get(timeout=TIMEOUT)
    ...
    def test_rebuild_nested_group_chord(self, manager):
        ...
        sig = chain(tasks.return_nested_signature_group_chord.s(), tasks.rebuild_signature.s())
        sig.delay().get(timeout=TIMEOUT)
    ...
    def test_rebuild_nested_chord_chain(self, manager):
        ...
        sig = chain(tasks.return_nested_signature_chord_chain.s(), tasks.rebuild_signature.s())
        sig.delay().get(timeout=TIMEOUT)
    ...
    def test_rebuild_nested_chord_group(self, manager):
        ...
        sig = chain(tasks.return_nested_signature_chord_group.s(), tasks.rebuild_signature.s())
        sig.delay().get(timeout=TIMEOUT)
    ...
    def test_rebuild_nested_chord_chord(self, manager):
        ...
        sig = chain(tasks.return_nested_signature_chord_chord.s(), tasks.rebuild_signature.s())
        sig.delay().get(timeout=TIMEOUT)
...
class test_stamping_mechanism:
    def test_stamping_workflow(self, manager, subtests):
        ...
        def before_task_publish_handler(
            ...
            **kwargs,
            ...
            assertion_result = all(stamped_header in headers["stamps"] for stamped_header in headers["stamped_headers"])
    ...
    def test_err_linking_stamped_sig(self, manager):
        ...
        def task_received_handler(sender=None, request=None, signal=None, **kwargs):
            ...
            assertion_result = all(
                stamped_header in link_error["options"] for stamped_header in link_error["options"]["stamped_headers"]
            )
    ...
    def test_stamp_canvas_with_dictionary_link_error(self, manager, subtests):
        ...
        with subtests.test(msg="Expect canvas to fail"), pytest.raises(ExpectedException):
