...
class test_chain:
    ...
    @pytest.mark.xfail(
        raises=TimeoutError, reason="Task is timeout instead of returning exception on rpc backend", strict=False
    )
...
class test_chord:
    ...
    def test_chord_on_error(self, manager):
        ...
        if manager.app.conf.result_backend_transport_options.get("result_chord_ordered", True):
