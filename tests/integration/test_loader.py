...
class test_loader:
    def test_autodiscovery__when_packages_exist(self, manager):
        ...
        assert not any(task.startswith(unexpected_package_name) for task in manager.app.tasks), (
    ...
    def test_autodiscovery__when_packages_do_not_exist(self, manager):
        ...
        with pytest.raises(ModuleNotFoundError) as exc:
            manager.app.autodiscover_tasks([existent_package_name, nonexistent_package_name], module_name, force=True)
